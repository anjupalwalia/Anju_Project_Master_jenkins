...
Library    DataDriver    file=${e2e_test_data_excel_file}      encoding=utf-8    dialect=unix    sheet_name=Needdate
Test Template    Amount Based yes Agreement Basic Roundtrip With changed Need Date

*** Test Cases ***
Amount Based yes Agreement Basic Roundtrip With changed Need Date
    [Tags]    Different_Commodity    Agreement_Yes    Fastrack    AP
...
Amount Based yes Agreement Basic Roundtrip With changed Need Date
    ...
    ...    ${second_spend_family_value}	${second_spend_group_value}    ${second_need_by_date_value}    ${second_service_end_date_value}
    ...
    And User is filling the fields and submit the PIR with multiple Line Item for changing Need by date     ${mandatory_commodity_value}    ${chart_of_accounts_value}    ${region_value}    
